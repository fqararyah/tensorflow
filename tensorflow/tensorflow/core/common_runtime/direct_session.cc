...
Status DirectSession::CreateExecutors(
  ...
  for (auto iter = graphs.begin(); iter != graphs.end(); ++iter) {
    ...
    /*
    printf("This is partition %s with %d nodes.\n\n", partition_name.c_str(), partition_graph.get()->num_nodes());
	*/
    ...
    optimizer.Optimize(lib, options_.env, device, &iter->second,
                       /*shape_map=*/nullptr);
    /*DEBUG-DOGA
    for(Node* node:iter->second.get()->nodes())
    {
    	//const string deviceTest = "/gpu:2";
        node->set_assigned_device_name("/gpu:2");
        /*
        if(node->id() == 3)
        {
           	if(checkIter != 1) {
           		node->set_requested_device("/job:localhost/replica:0/task:0/device:GPU:2");
           		//node->set_requested_device("/job:localhost/replica:0/task:0/device:GPU:2");
           	}
    ...
    /*Debug-DOGA
    for(Node* node:partition_graph.get()->nodes())
    {
    	printf("Node After: %s\n", node->assigned_device_name().c_str());
    }
    /*/
